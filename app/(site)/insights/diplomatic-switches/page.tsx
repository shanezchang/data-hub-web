...
export default function DiplomaticSwitchesPage() {
  return (
    <main className="mx-auto max-w-3xl px-5 py-16">
      ...
      <p className="mb-3 font-mono text-xs uppercase tracking-[0.18em] text-brand">Insight · Diplomacy</p>
        ...
        {data.countries.map((c) => (
          <section key={c.zh}>
            <h2 className="text-xl font-bold">
              {c.en} <span className="ml-1 font-normal text-muted-foreground">{c.zh}</span>
            </h2>
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              ...
              <a href={c.first.url} target="_blank" rel="noopener" className="text-brand hover:underline">
                {c.first.title}
              </a>{" "}
      ...
      <ul className="mt-3 space-y-1.5 text-sm text-muted-foreground">
        <li>· Counts: <code className="font-mono text-foreground">GET /v1/news?q=&lt;country name in Chinese&gt;&amp;group_by=year</code> — broadcast items containing the exact country name, from full transcripts of every Xinwen Lianbo since 2016.</li>
