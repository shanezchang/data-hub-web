  ...
  return (
    <main className="mx-auto max-w-3xl px-5 py-16">
        ...
        {data.decliners.map((d) => (
          <section key={d.zh}>
            <h3 className="text-lg font-bold">
              {d.en} <span className="ml-1 font-normal text-muted-foreground">{d.zh}</span>
            </h3>
            <p className="mt-1 text-sm text-muted-foreground">peak {d.peak} items in {d.peak_year}</p>
      ...
      <ul className="mt-3 space-y-1.5 text-sm text-muted-foreground">
        <li>· Counts: <code className="font-mono text-foreground">GET /v1/news?q=&lt;phrase&gt;&amp;group_by=year</code> — broadcast items containing the exact phrase, from full transcripts of every Xinwen Lianbo since 2016.</li>
      ...
      <p className="mt-10 rounded-lg border border-border bg-muted px-5 py-4 text-sm text-muted-foreground">
        ...
        {" · "}
        <Link href="/trends/xinwen-lianbo-keywords" className="text-brand hover:underline">keyword trends</Link>.
      </p>
    </main>
  );
}
