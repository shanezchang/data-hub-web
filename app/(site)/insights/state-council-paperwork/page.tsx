  ...
  return (
    <main className="mx-auto max-w-3xl px-5 py-16">
          ...
          <tbody>
            {years.map((y) => (
              <tr key={y} className="border-t border-border">
                ...
                {DOCTYPES.map((t) => (
                  <td key={t} className="px-3 py-1.5 font-mono text-muted-foreground">{byYear[y][t] ?? 0}</td>
                ))}
              </tr>
            ))}
          </tbody>
      ...
      <div className="mt-4 overflow-hidden rounded-lg border border-border">
        {DOCTYPES.map((t, i) => (
          <div key={t} className={`flex items-baseline gap-4 px-5 py-3 ${i ? "border-t border-border" : ""}`}>
            <code className="shrink-0 font-mono text-sm text-foreground">{t}</code>
            <span className="text-sm text-muted-foreground">{DOCTYPE_EN[t]} · {(data.overall as Record<string, number>)[t]} docs overall</span>
          </div>
        ))}
      </div>
      ...
      <ul className="mt-3 space-y-1.5 text-sm text-muted-foreground">
        <li>· Counts: <code className="font-mono text-foreground">POST /v1/policy/search</code> with{" "}
          <code className="font-mono text-foreground">{"{\"group_by\":[\"year\",\"doctype\"]}"}</code>; document type is extracted from the official serial number (发文字号).</li>
