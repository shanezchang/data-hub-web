    ...
    <main className="mx-auto max-w-3xl px-5 py-16">
          ...
          <tbody>
            {years.map((y) => (
              <tr key={y} className="border-t border-border">
                ...
                {data.industries.map((ind) => (
                  <td key={ind} className="px-3 py-1.5 font-mono text-muted-foreground">{byYear[y][ind] ?? 0}</td>
                ))}
              </tr>
            ))}
          </tbody>
      ...
      <ul className="mt-3 space-y-1.5 text-sm text-muted-foreground">
        <li>· One query: <code className="font-mono text-foreground">POST /v1/yc/companies/search</code> with{" "}
          <code className="font-mono text-foreground">{"{\"group_by\":[\"batch_year\",\"industry\"]}"}</code>.</li>
      ...
      <p className="mt-10 rounded-lg border border-border bg-muted px-5 py-4 text-sm text-muted-foreground">
        ...
        Related: <Link href="/trends/yc-batch-survival" className="text-brand hover:underline">batch survival rates</Link>
