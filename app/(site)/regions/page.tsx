...
export default async function RegionsIndexPage() {
  ...
  return (
    <main className="mx-auto max-w-3xl px-5 py-16">
      <p className="mb-3 font-mono text-xs uppercase tracking-[0.18em] text-accent">Regions</p>
      ...
      <div className="mt-10 space-y-4">
        {entries.map((r) => (
          <article key={r.slug} className="rounded-lg border border-line px-6 py-5">
            <h2 className="text-lg font-bold">
              <Link href={`/regions/${r.slug}`} className="hover:text-accent">{r.en} {r.zh}</Link>
            </h2>
      ...
      <p className="mt-12 text-sm text-muted">
        Related: <Link href="/insights" className="text-accent hover:underline">Data Insights</Link>
        {" · "}
        <Link href="/datasets/news" className="text-accent hover:underline">Xinwen Lianbo API</Link>
      </p>
    </main>
  );
}
