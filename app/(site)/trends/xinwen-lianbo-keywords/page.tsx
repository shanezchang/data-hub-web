...
export default function XwlbTrendsPage() {
  return (
    <main className="mx-auto max-w-3xl px-5 py-16">
      ...
      <p className="mb-3 font-mono text-xs uppercase tracking-[0.18em] text-brand">Trends · Xinwen Lianbo</p>
      ...
      <p className="mt-5 max-w-2xl leading-relaxed text-muted-foreground">
        ...
        <Link href="/datasets/news" className="text-brand hover:underline">data-hub Xinwen Lianbo dataset</Link>.
      </p>
        ...
        {trends.keywords.map((k) => (
          <section key={k.zh}>
            <h2 className="text-xl font-bold">
              {k.en} <span className="ml-1 font-normal text-muted-foreground">{k.zh}</span>
            </h2>
          ...
          <tbody>
            {Object.keys(trends.keywords[0].by_year).sort().map((yr) => (
              <tr key={yr} className="border-t border-border">
                ...
                {trends.keywords.map((k) => (
                  <td key={k.zh} className="px-3 py-1.5 font-mono text-muted-foreground">
      ...
      <ul className="mt-3 space-y-1.5 text-sm text-muted-foreground">
        ...
        <li>· Counts come from <code className="font-mono text-foreground">GET /v1/news?q=&lt;term&gt;&amp;group_by=year</code> on the data-hub API.</li>
      ...
      <p className="mt-10 rounded-lg border border-border bg-muted px-5 py-4 text-sm text-muted-foreground">
        ...
        via the <Link href="/datasets/news" className="text-brand hover:underline">Xinwen Lianbo full-text API</Link>.
        AI agents can start from{" "}
        <a href="https://api.lumina-core.cn/llms.txt" target="_blank" rel="noopener" className="text-brand hover:underline">llms.txt</a>.
      </p>
    </main>
  );
}
